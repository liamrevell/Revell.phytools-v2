...
*phytools* also contains methods to visualize our phylogenetic signal analyses. In particular, for Blomberg et al.'s *K* we can plot the permutation distribution of *K* as well as our observed measure; whereas for Pagel's $\lambda$ we can plot the likelihood surface and our measure solution. Both of these plots are illustrated in Figure \@ref{phylosig} for our *Anolis* body size data.
...
```{r cordylid-ace, fig.cap="Reconstructed ancestral values from Bayesian MCMC projected onto the nodes and edges of the tree."}
...
```{r cordylid-pd, fig.cap="Posterior probability density at node 49 of Figure  10from Bayesian MCMC ancestral state reconstruction of PC 1 from a morphological analysis on a phylogenetic tree of cordylid lizards."}
...
*phytools* contains several multivariate trait evolution models. One of these is a model (described in Revell and Collar, 2009; Revell et al., 2022) in which the rates and evolutionary correlation between traits is allowed to vary as a function of a set of mapped regimes on the phylogeny.

To illustrate this method, we'll use a phylogeny and dataset of tropidurid lizard species from Revell et al. (2022).
...
```{r trop-tree, fig.cap="Phylogenetic tree of tropidurid lizard species from Revell et al. (2022)."}
...
plot(tropidurid.tree,colors=cols,direction="upwards",outline=TRUE,lwd=3,
  fsize=0.5,ftype="i",offset=1)
legend("bottomright",c("non-rock dwelling","rock-dwelling"),pch=22,pt.bg=cols,
  cex=0.8,pt.cex=1.2)
```

Our data consist of a single measure of overall body size (as trait 1), and a second metric trait measuring dorsoventral flattening. Our hypothesis is that these two traits should scale together in non-rock dwelling lizard species: bigger lizards also tend to be deeper. However, we would like to ask if this general relationship is uncoupled in rock-dwelling species in which the force of selection favors *increased* flattening relative to their non-rock kin.

To test this, we'll use the *phytools* function `evolvcv.lite` which fits a heirarchical set of models for the evolutionary rates (of each character) and evolutionary correlation (between them).
...
primate.data<-read.csv(file="http://www.phytools.org/Rbook/3/primateEyes.csv",row.names=1)
```
```{r, eval=FALSE}
data(primate.tree)
data(primate.data)
...
```{r}
primate.multirateBM<-multirateBM(primate.tree,primate.lnSkull,lamba=1,
  parallel=TRUE)
```

```{r fig14, fig.cap="Estimated rates of log(body mass) evolution under a variable-rate Brownian evolution model."}
plot(primate.multirateBM,fsize=0.4,lwd=3,outline=FALSE,ftype="i")
```

# Diversification

In addition to the methods that we've seen so far, *phytools* also contains a handful of different techniques for investigating diversification.
