...
```{r liol-mccr, fig.cap="Lineage through time plot for phylogeny of lizards from the South American family Liolaemidae.",fig.height=4}
plot(liolaemid.mccr,las=1,cex.lab=0.9,cex.axis=0.8)
```
...
data(mammal.tree)
data(mammal.geog)
chk<-geiger::name.check(mammal.tree,mammal.geog)
mammal.pruned<-drop.tip(mammal.tree,chk$tree_not_data)
mammal.map<-phylo.to.map(mammal.pruned,mammal.geog,
  plot=FALSE,quiet=TRUE)
plot(mammal.map,fsize=0.6,ftype="i",
  colors=palette()[4])
```
